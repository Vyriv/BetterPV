...
public final class MuseumPage {
	...
	public record Slot(String donationId, InventoryDecoder.Stack stack, boolean missing, String coveredBy) {
		public Slot(String donationId, InventoryDecoder.Stack stack, boolean missing) {
			this(donationId, stack, missing, null);
		}
	...
	public void applyMuseum(JsonObject member) {
		...
		this.error = "";
		this.museumMember = member == null ? new JsonObject() : member;
		this.gridScroll = 0;
		this.listScroll = 0;
		this.cacheDirty = true;
	}
		...
		if (this.sort == MuseumSort.SPECIAL) {
			for (var entry : donated.entrySet()) {
				...
				if (key != null && key.toUpperCase(Locale.ROOT).startsWith("SPECIAL:")) {
					out.add(new Slot(key, res.stack(), false, null));
				} else if (!MuseumCatalog.isCataloged(key)) {
					out.add(new Slot(key, res.stack(), false, res.coveredBy()));
				}
			}
			for (String id : MuseumCatalog.donationIds(MuseumSort.SPECIAL)) {
				MuseumCatalog.ResolvedDonation res = donated.get(id);
				if (res == null) {
					out.add(new Slot(id, null, true, null));
				} else {
					out.add(new Slot(id, res.stack(), false, res.coveredBy()));
				}
			}
			return out;
		}
