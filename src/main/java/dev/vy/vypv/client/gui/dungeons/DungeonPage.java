package dev.vy.vypv.client.gui.dungeons;

import dev.vy.vypv.client.data.DungeonSnapshot;
import dev.vy.vypv.client.data.FormatUtil;
import dev.vy.vypv.client.dungeons.CataXpCalculator;
import dev.vy.vypv.client.dungeons.ClassLevelQuery;
import dev.vy.vypv.client.dungeons.ClassXpCalculator;
...
public final class DungeonPage {
	...
	public void runClassCalc() {
		...
		if (query == null) {
			this.overlay.open(DungeonCalcOverlay.View.error(
				"vypv.dungeons.calc.class",
				Component.translatable("vypv.dungeons.calc.class_invalid").getString()
			));
			return;
		}
	...
	private static DungeonCalcOverlay.View toView(CataXpCalculator.Result result) {
		...
		return DungeonCalcOverlay.View.single(
			"vypv.dungeons.calc.result_title",
			result.targetLevel(),
			result.xpNeeded(),
	...
	private static DungeonCalcOverlay.View toView(ClassXpCalculator.Result result) {
		...
		return DungeonCalcOverlay.View.single(
			"vypv.dungeons.calc.class_result_title",
	...
	private static DungeonCalcOverlay.View toView(ClassXpCalculator.AverageResult result) {
		...
		return DungeonCalcOverlay.View.average(
			"vypv.dungeons.calc.ca_result_title",
			titleArg,
			result.modsLabel() == null ? "" : result.modsLabel(),
			blocks,
			result.totalRuns(),
	...
	private void drawCalcPanel(
		...
		String header = Component.translatable("vypv.dungeons.calc.header").getString();
		...
		String cataLabel = Component.translatable("vypv.dungeons.calc.level_wanted").getString();
		String classLabel = Component.translatable("vypv.dungeons.calc.class_wanted").getString();
	...
	private void drawCalcRow(
		...
		String calc = Component.translatable("vypv.dungeons.calc.button").getString();
		PvDraw.textCentered(
			g, font, calc,
			row.btnX + row.btnW / 2,
	...
	private void drawRunsPanel(GuiGraphicsExtractor g, Font font, int x, int y, int w, int h, FloorLayout floors) {
		...
		String normalHeader = Component.translatable("vypv.dungeons.normal").getString();
		String masterHeader = Component.translatable("vypv.dungeons.master").getString();
		...
		for (int floor = 1; floor <= 7; floor++) {
			...
			String floorLabel = Component.translatable("vypv.dungeons.floor", floor).getString();
	...
	private void drawTotalLine(
		...
		String label = Component.translatable("vypv.dungeons.total").getString();
