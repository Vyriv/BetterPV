...
/** Parses inputs like {@code Mage 50}, {@code Bers 30}, {@code CA 50}, {@code Class Avg 50}. */
public final class ClassLevelQuery {
	...
	private static final Pattern CLASS_THEN_LEVEL = Pattern.compile("^([a-z]+)\\s*(\\d{1,2})$");
	private static final Pattern LEVEL_THEN_CLASS = Pattern.compile("^(\\d{1,2})\\s*([a-z]+)$");
	...
	public static Parsed parse(String raw) {
		...
		if (isAverageAlias(alias)) {
			return Parsed.ofAverage(clampLevel(level));
		}
		String classId = resolveAlias(alias);
		if (classId == null) {
			return null;
		}
		return Parsed.ofClass(classId, clampLevel(level));
	}

	private static Parsed parseAverage(String cleaned) {
		Matcher ca = CA_THEN_LEVEL.matcher(cleaned);
		if (ca.matches()) {
			return Parsed.ofAverage(clampLevel(Integer.parseInt(ca.group(1))));
		}
